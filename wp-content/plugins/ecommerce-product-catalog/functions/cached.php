...
function ic_process_current_products(
	...
	if ( is_array( $exclude ) && is_array( $exclude_meta ) && is_array( $exclude_tax_val ) ) {
		$cache_key          = 'current_products' . implode( '_', $exclude ) . json_encode( $exclude_tax ) . implode( '_', $exclude_meta ) . implode( '_', $exclude_tax_val ) . intval( $replace_post_in );
	...
	if ( $replace_post_in && ! empty( $args['ic_pre_post__in'] ) ) {
	...
	if ( empty( $wp_query->query['pagename'] ) && $wp_query->max_num_pages <= 1 && is_array( $wp_query->posts ) && empty( $exclude ) && empty( $exclude_tax ) && empty( $exclude_meta ) ) {
		if ( empty( $wp_query->posts ) ) {
